    ...
    <el-dialog title="申请详情" v-model="viewOpen" width="800px" append-to-body>
      <el-descriptions :column="2" border>
        ...
        <el-descriptions-item label="服务种类">
          <div v-if="viewForm.serviceTypeXx">
            {{ getServiceTypeName(viewForm.serviceTypeXx) }}
          </div>
          <div v-else style="color: #909399;">暂无</div>
        </el-descriptions-item>
        <el-descriptions-item label="服务区域">
          <span v-if="viewForm.regionPre">
            {{ regionDisplayTexts.get(viewForm.regionPre) || viewForm.regionPre }}
          </span>
          <span v-else-if="viewForm.regionId">
            {{ regionDisplayTexts.get(viewForm.regionId) || viewForm.regionId }}
          </span>
...
<script setup name="MyApplication">
import { ref, reactive, toRefs, getCurrentInstance, onMounted } from 'vue';
import useUserStore from '@/store/modules/user'
import { getProcess, updateProcess, delProcess } from '@/api/apply/process.js';
import { listServicer } from '@/api/subject/servicer.js';
import { mergeFruitData } from '@/utils/service-type.js';
import { getRegionTextByPath } from '@/utils/region.js';
import { listType } from '@/api/support/type.js';
...
// 地区文本映射
const regionDisplayTexts = ref(new Map());
// 服务类型数据
const serviceTypeList = ref([]);
const serviceTypeDisplayTexts = ref(new Map());
...
function initPage() {
  // 绑定仅查看当前用户申请
  queryParams.value.applyUserId = userStore.id
  loadServiceTypeData();
  getList();
}
...
function handleView(row) {
  getProcess(row.id).then(response => {
    viewForm.value = response.data;
    viewOpen.value = true;
    
    // 加载地区文本
    loadRegionText(response.data);
  });
}
...
function loadServiceTypeData() {
  if (serviceTypeList.value.length === 0) {
    listType({
      ...
      serviceTypeList.value = proxy.handleTree(response.data, "id", "pid");
    }).catch(error => {
      console.warn('加载服务类型数据失败:', error);
    });
  }
}

/** 根据服务类型ID路径获取服务类型名称 */
function getServiceTypeName(serviceTypeXx) {
  if (!serviceTypeXx || !serviceTypeList.value.length) {
    return serviceTypeXx || '暂无';
  }
  
  // 如果已经有缓存，直接返回
  if (serviceTypeDisplayTexts.value.has(serviceTypeXx)) {
    return serviceTypeDisplayTexts.value.get(serviceTypeXx);
  }
  
  try {
    // 解析服务类型路径，格式如 "4,7,8"
    const paths = serviceTypeXx.split(',');
    const names = [];
    
    for (let i = 0; i < paths.length; i++) {
      const id = parseInt(paths[i].trim());
      if (isNaN(id)) continue;
      
      if (i === 0) {
        // 第一层
        const firstLevel = serviceTypeList.value.find(item => item.id === id);
        if (firstLevel) {
          names.push(firstLevel.name);
        }
      } else if (i === 1) {
        // 第二层
        const firstLevel = serviceTypeList.value.find(item => item.id === parseInt(paths[0]));
        if (firstLevel && firstLevel.children) {
          const secondLevel = firstLevel.children.find(item => item.id === id);
          if (secondLevel) {
            names.push(secondLevel.name);
          }
        }
      } else if (i === 2) {
        // 第三层
        const firstLevel = serviceTypeList.value.find(item => item.id === parseInt(paths[0]));
        if (firstLevel && firstLevel.children) {
          const secondLevel = firstLevel.children.find(item => item.id === parseInt(paths[1]));
          if (secondLevel && secondLevel.children) {
            const thirdLevel = secondLevel.children.find(item => item.id === id);
            if (thirdLevel) {
              names.push(thirdLevel.name);
            }
          }
        }
      }
    ...
    const result = names.length > 0 ? names.join(' - ') : serviceTypeXx;
    serviceTypeDisplayTexts.value.set(serviceTypeXx, result);
    return result;
  } catch (error) {
    console.warn('解析服务类型失败:', error);
    serviceTypeDisplayTexts.value.set(serviceTypeXx, serviceTypeXx);
    return serviceTypeXx;
  }
}

/** 加载地区文本 */
function loadRegionText(data) {
  // 优先处理 regionPre（地区路径字符串）
  if (data.regionPre && typeof data.regionPre === 'string' && !regionDisplayTexts.value.has(data.regionPre)) {
    try {
      // regionPre 格式如 "1/72/2799"，需要转换为地区名称
      const ids = data.regionPre.split('/').map(id => parseInt(id.trim())).filter(id => !isNaN(id));
      
      if (ids.length > 0) {
        getRegionTextByPath(ids).then(text => {
          regionDisplayTexts.value.set(data.regionPre, text);
        }).catch(error => {
          console.warn('获取地区文本失败:', error);
          regionDisplayTexts.value.set(data.regionPre, data.regionPre);
        });
      }
    } catch (error) {
      console.warn('解析regionPre失败:', error);
      regionDisplayTexts.value.set(data.regionPre, data.regionPre);
    }
  }
  
  // 处理 regionId（如果存在）
  if (data.regionId && !regionDisplayTexts.value.has(data.regionId)) {
    if (typeof data.regionId === 'string') {
      try {
        // 尝试不同的分隔符
        let ids = [];
        if (data.regionId.includes(',')) {
          ids = data.regionId.split(',').map(id => parseInt(id.trim())).filter(id => !isNaN(id));
        } else if (data.regionId.includes('/')) {
          ids = data.regionId.split('/').map(id => parseInt(id.trim())).filter(id => !isNaN(id));
        } else {
          // 尝试直接解析为单个ID
          const singleId = parseInt(data.regionId);
          if (!isNaN(singleId)) {
            ids = [singleId];
          }
        }
        ...
        if (ids.length > 0) {
          getRegionTextByPath(ids).then(text => {
            regionDisplayTexts.value.set(data.regionId, text);
          }).catch(error => {
            console.warn('获取地区文本失败:', error);
            regionDisplayTexts.value.set(data.regionId, data.regionId);
          });
        } else {
          regionDisplayTexts.value.set(data.regionId, data.regionId);
        }
      } catch (error) {
        console.warn('解析地区ID失败:', error);
        regionDisplayTexts.value.set(data.regionId, data.regionId);
      }
    } else if (Array.isArray(data.regionId)) {
      // 如果regionId是数组
      const ids = data.regionId.map(id => parseInt(id)).filter(id => !isNaN(id));
      if (ids.length > 0) {
        getRegionTextByPath(ids).then(text => {
          regionDisplayTexts.value.set(JSON.stringify(data.regionId), text);
        }).catch(error => {
          console.warn('获取地区文本失败:', error);
          regionDisplayTexts.value.set(JSON.stringify(data.regionId), JSON.stringify(data.regionId));
        });
      }
    }
  }
...
function submitEditForm() {
  proxy.$refs['editRef'].validate(valid => {
    if (valid) {
      updateProcess(editForm.value).then(response => {
